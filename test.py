...
def print_trian_info(*trains_list):	
...
#根据值找键
	...
	for train in trains_list:
		...
		x.add_row([train['train'],from_station_name+"--"+to_station_name,
							train['start_time']+"--"+train['end_time'],
							train['time_consuming'],train['A9'],train['P'],train['M'],train['O'],train['A6'],
							train['A4'],train['A3'],train['A1'],train['WZ']])
	print(x)
...
#url = 'https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=2017-05-25&leftTicketDTO.from_station=SHH&leftTicketDTO.to_station=CQW&purpose_codes=ADULT'
...
def search_train(url):
	...
	for item in results:
		...
		#pprint(trains_info)
		#get_seat_info(trains_info['train_no'],trains_info['from_station_no'],trains_info['to_station_no'],trains_info['seat_types'],trains_info['train_date'],**seat_info)
	
	print_trian_info(*trains_list)
